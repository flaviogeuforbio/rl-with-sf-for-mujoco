...
def smooth_and_pad(data_list, window=20): 
    """
    Applies a moving average to each run and truncates them to the minimum length.
    This naturally bounds the x-axis to the episode count of the fastest successful seed.
    """
    if not data_list: 
        return np.array([]) 
    
    # Apply moving average to each run
    smoothed_data = [np.convolve(run, np.ones(window)/window, mode='valid') for run in data_list] 
    
    # Find the minimum length among the smoothed runs to restrict the x-axis
    min_len = min(len(run) for run in smoothed_data) 
    
    # Truncate each smoothed run to the minimum length
...
def generate_transfer_comparison_plot(seq_dir, scratch_dir, gamma, main_title, window_size=20):
    ...
    sf_seq_p0, sf_seq_p1 = load_data(seq_dir, "sf_ddpg", run_kind="transfer") 
    base_seq_p0, base_seq_p1 = load_data(seq_dir, "ddpg", run_kind="transfer") 
    
    sf_scratch_p0, _ = load_data(scratch_dir, "sf_ddpg", run_kind="scratch") 
    base_scratch_p0, _ = load_data(scratch_dir, "ddpg", run_kind="scratch") 
...
if __name__ == "__main__":
    ...
    # Format the parameters to match the folder naming convention
