...
def train_sf_ddpg(
    ...
    # Extract dimensions from Cheetah (Walker has the exact same dimensions)
    ...
    for phase in phases:
        ...
        # 2. Seed the specific environment for this phase
        ...
        # Reset Replay Buffer on task switch (as per Chua 2024 distribution shift protocol, see Figure 4 Caption in the paper.)
        replay_buffer.clear()
        
        if phase == phases[0]: # If it's the first phase, we reset the environment with a fixed seed for reproducibility. This works also for the walker_only case, as it will be the only phase. It would not work if we write `if phase == 0:` because in the walker_only case, phase would be 1, and we would not reset with a seed.
        ...
        for step in range(steps_per_phase):
            ...
            # Both Phase 0 (Cheetah) and Phase 1 (Walker) are FORWARD tasks
            ...
            if len(replay_buffer.storage) >= WARMUP_STEPS:
                ...
                # ---------------------------------------------------------
                with torch.no_grad():
                    # Both phases target forward velocity + control reward
            ...
            if done:
                state, _ = env.reset()
                # Cast to standard Python float to allow JSON serialization
...
def train_ddpg(
    ...
    # Both tasks use w_forward! The difference is the environment.
    if walker_only:
        tasks = [
            {"name": "Task 2 (Walker Forward) from Scratch", "w": w_forward, "phase_idx": 1}
        ]
    else:
        tasks = [
...
def parse_arg():
    ...
    parser.add_argument("--lambda_vec", type=float, default=1.0, help = "Vectorial TD loss (standard SF-TD) term weight")
    ...
    parser.add_argument("--walker_only", action="store_true", help="Train ONLY Walker2d-v5 from scratch")
...
if __name__ == "__main__":
    ...
    # Modified to save inside a "walker" subfolder
