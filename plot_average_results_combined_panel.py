...
def generate_walker_transfer_plot(
    ...
    _, sf_transfer_returns = load_data(seq_dir, "sf_ddpg")
    _, ddpg_transfer_returns = load_data(seq_dir, "ddpg")

    # Scratch runs: Walker2d is the only phase, therefore data[0].
    sf_scratch_returns, _ = load_data(scratch_dir, "sf_ddpg")
    ddpg_scratch_returns, _ = load_data(scratch_dir, "ddpg")
...
def generate_cheetah_walker_combined_plot(
    ...
    # One figure with two side-by-side panels and a shared return axis.
...
if __name__ == "__main__":
    ...
    if args.plot_combined:
        if args.seq_dir is None:
            raise ValueError(
                "--seq_dir is required with --plot_combined. "
                "It must directly contain the transfer seed_* folders."
            )

        if args.scratch_dir is None:
            raise ValueError(
                "--scratch_dir is required with --plot_combined. "
                "It must directly contain the Walker-from-scratch "
                "seed_* folders."
            )

        SEQ_DIR = os.path.normpath(args.seq_dir)
        SCRATCH_DIR = os.path.normpath(args.scratch_dir)
    ...
    if args.plot_walker:
        if args.seq_dir is None:
            raise ValueError(
                "--seq_dir is required with --plot_walker. "
                "It must directly contain the transfer seed_* folders."
            )

        if args.scratch_dir is None:
            raise ValueError(
                "--scratch_dir is required with --plot_walker. "
                "It must directly contain the Walker-from-scratch seed_* folders."
            )
